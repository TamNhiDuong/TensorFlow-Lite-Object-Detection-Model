...
This downloads everything into a folder called TensorFlow-Lite-Object-Detection-Model. That's a little long to work with, so rename the folder to "tflite1" and then cd into it (Renaming is optional, but for this guide we are using the renamed one):

```
mv TensorFlow-Lite-Object-Detection-Model tflite1
cd tflite1
```
...
```
```
python3 TFLite_detection_stream.py --modeldir=Sample_TFLite_model --streamurl=STREAMURL --resolution=640x480
```

The resolution is only necessary when the stream resolution is not the default "1280x720"
